...
class BaseAviary(gym.Env):
    ...
    def __init__(self,
        ...
        #### Set the parameters for global map ######################
        # print(global_params)
        self.LENGTH = global_params["length"]
        self.RANGE_X = global_params["range_x"]
        self.RANGE_Y = global_params["range_y"]
        self.RANGE_Z = global_params["range_z"]
        #### Options ###############################################
    ...
    def _addObstacles(self):
        ...
        """
        # make a dic to store the obstacles
        obstacle_dic = {}
        ...
        self.load_calculate_map()
        """
        p.loadURDF("../assets/small_box.urdf",
                   [.5, .5, 0.25],
        ...
        self._CalculateMap(shape="box", position=[0.5,0.5,0.25], size="s")
        ...
        # update the global map
        self._CalculateMap(shape="box", position=[0.5,0.5,0.75], size="s")
    ...
    def _CalculateMap(self,shape,position,size):
        """
        Calculates the global map of the environment
        ...
        ### Get the shape and size of the obstacle
        # shape: box
        if (shape == "box") & (size == "s"):
            # update the global map
            # Area of the obstacle
            x_range = int(0.25/self.LENGTH)
            y_range = int(0.25/self.LENGTH)
            z_range = int(0.25/self.LENGTH)
            # Swollen the obstacle
            x_range_large = int((0.25+0.1)/self.LENGTH)
            y_range_large = int((0.25+0.1)/self.LENGTH)
            z_range_large = int((0.25+0.1)/self.LENGTH)
            # Center of the obstacle
            center_x = int(position[0]/self.LENGTH)
            center_y = int(position[1]/self.LENGTH)
            center_z = int(position[2]/self.LENGTH)

            # Update the global map
            for i in range(-x_range,x_range+1):
                for j in range(-y_range,y_range+1):
                    for k in range(-z_range,z_range+1):
                        global_all["map"][center_x+i][center_y+j][center_z+k] = self.NUM_DRONES+2       # The true space of the obstacle

            for i in range(-x_range_large,x_range_large+1):
                for j in range(-y_range_large,y_range_large+1):
                    for k in range(-z_range_large,z_range_large+1):
                        if global_all["map"][center_x+i][center_y+j][center_z+k] != self.NUM_DRONES+2:  
                            global_all["map"][center_x+i][center_y+j][center_z+k] = self.NUM_DRONES+1   # The expanded space
            
            #update the global obstacle list
            global_obstacles.append({"center": [center_x,center_y,center_z],
                                     "size": [x_range,y_range,z_range],
                                     "shape": "box"})

        # shape: cuboid
        if (shape == "cuboid") & (size == "s"):
            # update the global map
            # Area of the obstacle
            x_range = int(0.1/self.LENGTH)
            y_range = int(0.1/self.LENGTH)
            z_range = int(1.5/self.LENGTH)
            # Swollen the obstacle
            x_range_large = int((0.1+0.1)/self.LENGTH)
            y_range_large = int((0.1+0.1)/self.LENGTH)
            z_range_large = int((1.5+0.1)/self.LENGTH)
            # Center of the obstacle
            center_x = int(position[0]/self.LENGTH)
            center_y = int(position[1]/self.LENGTH)
            center_z = int(position[2]/self.LENGTH)
        ...
        # shape: sphere
        if (shape == "sphere") & (size == "s"):
            # update the global map
            # Area of the obstacle
            r_range = int(0.2/self.LENGTH)
            r_range_larger = int(0.2+0.2/self.LENGTH)
            # Center of the obstacle
            center_x = int(position[0]/self.LENGTH)
            center_y = int(position[1]/self.LENGTH)
            center_z = int(position[2]/self.LENGTH)
            ...
            for i in range(-r_range,r_range+1):
                r_layer = np.sqrt(int(0.2/self.LENGTH)**2 - i**2)
                r_range_list = np.linspace(0,r_layer,5)
                for j in range(5):
                    theta_list = np.linspace(-np.pi,np.pi,100)
                    y_list = r_range_list[j]*np.cos(theta_list)
                    z_list = r_range_list[j]*np.sin(theta_list)
                    for k in range(len(y_list)):
                        global_all["map"][center_x+i][center_y+round(y_list[k])][center_z+round(z_list[k])] = self.NUM_DRONES+2     # The true space of the obstacle
            for i in range(-r_range_larger,r_range_larger+1):
                r_layer = np.sqrt(int(0.2/self.LENGTH)**2 - i**2)
                r_range_list = np.linspace(0,r_layer,5)
                for j in range(5):
                    theta_list = np.linspace(-np.pi,np.pi,100)
                    ...
                    z_list = r_range_list[j]*np.sin(theta_list)
                    for k in range(len(y_list)):
                        if global_all["map"][center_x+i][center_y+round(y_list[k])][center_z+round(z_list[k])] != self.NUM_DRONES+2:
                            global_all["map"][center_x+i][center_y+round(y_list[k])][center_z+round(z_list[k])]!=self.NUM_DRONES+1  # The expanded space
    ...
    # define a function to load the obstacle and show the map
    def load_calculate_map(self):
        """
        Loads the obstacles and calculates the global map of the environment

        Changed:
        ----------
        Env and global_all["map"]
