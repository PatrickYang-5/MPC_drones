...
class RRT_star():
    ...
    def rrt_star(self):
        ...
        detect_range = 3 # the range of the node to detect its parents
        end_size = 2 # the range to end the loop
        threshold = 30 # the range to get new points
        steps = 100 # the maximiun step of the loop
        ...
        for j in range(steps):
            ...
            new_node.parents = self.get_parent(parents_node, start_node)
    ...
    def get_parent(self,parent_node,start_node):
        ...
        for i in range(len(parent_node)-1):
            distance_to_start = self.distance(parent_node[i+1],start_node)
            if distance_to_start < distance:
                distance = distance_to_start
                num = i+1
        return parent_node[num]
