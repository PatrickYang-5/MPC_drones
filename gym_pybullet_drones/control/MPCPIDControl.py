...
class MPCPIDControl(BaseControl):
    ...
    def computeControl(self,
        ...
        print("thrust", thrust)
        print("computed_target_rpy", computed_target_rpy)
        print("F[0]", F[0])
        print("F[1:3]", F[1:4])
    ...
    ################################################################################
    def computeControl_moving(self,
                       control_timestep,
                       cur_pos,
                       cur_quat,
                       cur_vel,
        ...
        """
        self.control_counter += 1
        thrust, computed_target_rpy, pos_e = self._dslPIDPositionControl_moving(control_timestep,
                                                                         cur_pos,
                                                                         cur_quat,
                                                                         cur_vel,
                                                                         target_pos,
    ...
    ################################################################################
    def _dslPIDPositionControl_moving(self,
                               control_timestep,
                               cur_pos,
                               cur_quat,
                               cur_vel,
    ...
    #define a function to calculate the power of the potential field
    def create_potential_field(self,cur_pos,obs_pos,range,intensity):

        distance_to_potential = cur_pos-obs_pos
        distance_unit = distance_to_potential/np.linalg.norm(distance_to_potential)
        dis = np.linalg.norm(distance_to_potential)

        # the potential field only works in a given range
        if dis<range:
            potential_vector = intensity*distance_unit*(1/dis**2-1/range**2)
        else:
            potential_vector = np.zeros([3])
