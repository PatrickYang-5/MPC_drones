...
def run(
    ...
    # Initializations for 2 drones (with path conflicts)
    ...
    for i in range(MAX_STEP):
        ...
        for j in range(num_drones):
            ...
            # Get the next horizon period positions used in MPC based on OTS
            generated_pos_period = np.hstack([TARGET_POS[wp_counters[j]:(wp_counters[j]+MPC_N+1), 0:3, j]])

            # Get the target states used in MPC with
            state_target = np.hstack([generated_pos_period, np.zeros((MPC_N+1,9))])
            # Calculate the optimized force and state based on the MPC controller
